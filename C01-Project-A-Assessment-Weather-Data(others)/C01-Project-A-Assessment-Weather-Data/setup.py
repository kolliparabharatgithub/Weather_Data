...
#     'createRole':'admin', 
#     'privileges':[
#         {'resource':{ 'db':DB_NAME, 'collection':''}, 'actions':['find','insert','remove','update']}
#         ],
#     'roles':[]
#     })
# weather_dbh.command({ 
#     'createRole':'default', 
#     'privileges':[
#         {'resource':{ 'db':DB_NAME, 'collection':''}, 'actions':['find']}
#         ],
#     'roles':[]
#     })
...
# Also inserts dummy device access data for project usage.
with open(RELATIVE_CONFIG_PATH+USER_COLLECTION+'.csv', 'r') as user_fh:
    number = int(random.normalvariate(3,3))
    print(number)
    for user_row in user_fh:
        ...
        user_data = {
            ...
            'role': role,
            'alist':[
                {'did': ('D'+('T' if(number%2==0) else 'H')+'00'+str(number%2 + 1)), 'atype': ('r' if(number%3==0) else 'rw')}, 
                {'did': ('D'+('T' if(number%3==0) else 'H')+'00'+str(number%3 + 1)), 'atype': ('r' if(number%2==0) else 'rw')}, 
            ]
            }
...
#aggregator for already present data for Reports DB
