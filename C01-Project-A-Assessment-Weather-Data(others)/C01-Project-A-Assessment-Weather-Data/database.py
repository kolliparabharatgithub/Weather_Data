...
class Database:
    ...
    # This method finds all documents using field information provided in the key parameter
    # It assumes that the key returns a unique document. It returns None if no document is found
    ...
    # This method gets all the documents of a collection
    def get_all_documents(self, collection):
        db_collection = self._db[collection]
        return db_collection.find({})

    def get_all_aggregated_data(self, collection, keys):
        db_collection = self._db[collection]
        return db_collection.aggregate(keys)
